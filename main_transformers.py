...
def main():
    if config.TRAIN.USE_CHECKPOINT:
        ...
        checkpoint_name = r'E:\LAB\result_files\test_results\Aset_LR_segb5\best_model.pth'
    ...
    # 2) test를 위해 베스트 체크포인트 로드(모델 정보와 epoch 정보만 불러온다, 학습 재개를 위한다면 코드 수정필요)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = train_transformers.model
    ...
    test_iou_path = config.GENERAL.LOG_DIR / f"test_iou_epoch_{best_epoch}.csv"
    ...
    plt.ylabel("True")
    plt.tight_layout()
    test_cm_path = config.GENERAL.LOG_DIR / f"test_confusion_matrix_epoch_{best_epoch}.png"
    ...
    for filename in random.sample(all_imgs, k):
