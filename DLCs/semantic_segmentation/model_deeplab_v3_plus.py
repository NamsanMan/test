...
class InvertedResidual(nn.Module):
    ...
    def forward(self, x):
        if self.use_res_connect:
            return x + self.conv(F.pad(x, [1, 1, 1, 1], mode='constant', value=0))
        else:
            return self.conv(F.pad(x, [1, 1, 1, 1], mode='constant', value=0))

class MobileNetV2(nn.Module):
    def __init__(self, in_channels=3, output_stride=16, width_mult=1.0, pretrained=True, BatchNorm=nn.BatchNorm2d):
        super(MobileNetV2, self).__init__()

        # MobileNetV2 configuration (expand ratio, channels, number of layers, stride)
        ...
        self.layer0 = conv_bn(in_channels, input_channel, stride=2, BatchNorm=BatchNorm)
        current_stride *= 2
        ...
        for t, c, n, s in interverted_residual_setting:
            ...
            for i in range(n):
                if i == 0:
                    layers.append(InvertedResidual(input_channel, output_channel, stride, dilation, t, BatchNorm))
                else:
                    layers.append(InvertedResidual(input_channel, output_channel, 1, dilation, t, BatchNorm))
                input_channel = output_channel

            self.features.append(nn.Sequential(*layers))

        # Converting the feature list to Sequential
        ...
        # Separate into low-level and high-level features
        self.low_level_features = nn.Sequential(self.layer0, *self.features[:2])
        self.high_level_features = nn.Sequential(*self.features[2:])
...
class DeepLab_v3_plus(BaseModel):
    def __init__(self, num_classes, in_channels=3, backbone='xception', pretrained=False,
        ...
        assert ('xception' or 'resnet' or 'mobilenet' in backbone)
        if 'resnet' in backbone:
            ...
            self.backbone = MobileNetV2(in_channels=in_channels, output_stride=output_stride, pretrained=pretrained,  BatchNorm=nn.BatchNorm2d)
            low_level_channels = 24  # MobileNetV2의 저수준 특징 맵의 채널 수
            self.ASSP = ASSP(in_channels=320, output_stride=output_stride)


        # self.ASSP = ASSP(in_channels=2048, output_stride=output_stride)
        # mobileNetV2의 경우, 저수준 특징 맵의 채널 수를 24로 설정
